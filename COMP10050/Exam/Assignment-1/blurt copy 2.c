...
void parallelSwap(char songList[][MAX_LENGTH], char artistList[][MAX_LENGTH], int i, int j) {
...
void shufflePlaylist(char songList[][MAX_LENGTH], char artistList[][MAX_LENGTH], int numOfSongs) {
    ...
    for (int i = totalItems - 1; i > 0; i--) {
        int j = rand() % (i + 1);
        parallelSwap(poolSongs, poolArtists, i, j);
    }
